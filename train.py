...
parser.add_argument('--checkpoint_freqs', type=int, default=200,
                    help='Epoch frequency for model checkpointing.')
parser.add_argument('--image_freqs', type=int, default=10,
                    help='Frequency of logging gifs.')
...
if __name__ == "__main__":
    ...
    train_frames_0 = frames_from_video_file(os.path.join(args.ground_truth_train_dir, 'scene_2007'),
                                        os.path.join(args.tecogan_generated_train_dir, 'scene_2007'),
                                        args.sequence_length,
                                        (args.lr_height, args.lr_width),
                                        x1 = 25, x2 = 40, start = 0)

    train_frames_1 = frames_from_video_file(os.path.join(args.ground_truth_train_dir, 'scene_2053'),
                                          os.path.join(args.tecogan_generated_train_dir, 'scene_2053'),
                                          args.sequence_length,
                                          (args.lr_height, args.lr_width),
                                          x1=81, x2=193, start=0)
    ...
    groundtruth_images = np.moveaxis((np.stack((train_frames_0[1], train_frames_1[1], val_frames[1]),
                                               axis = 0) * 255).astype(np.uint8), -1, 2)
    teco_images = np.moveaxis((np.stack((train_frames_0[2], train_frames_1[2], val_frames[2]),
                                               axis=0) * 255).astype(np.uint8), -1, 2)
    log({'groundtruth_images': wandb.Video(groundtruth_images),
         'teco_images': wandb.Video(teco_images)}, step=0)

    frames = np.stack((train_frames_0[0], train_frames_1[0], val_frames[0]), axis = 0)
